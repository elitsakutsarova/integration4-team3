...
export async function clientLoader({ request }) {
  ...
  const initialRoom = rawRoom ? validateConnectRoom(rawRoom).value ?? null : null;
  return {
    initialRoom,
...
export default function ConnectPage() {
  ...
  const connect = useCallback(async () => {
    ...
    const roomResult = validateConnectRoom(room);
    if (roomResult.field) {
      setStatus('error');
      setMessages([{ from: 'system', text: roomResult.message }]);
      return;
    }

    try {
      const session = await connectToRoom(roomResult.value, {
        onOpen: () => {
          setStatus('connected');
        ...
        onMessage: payload => {
          ...
          const messageResult = validateConnectMessage(rawText);
          const text = messageResult.value ?? '[message blocked]';
  ...
  function sendMessage(event) {
    ...
    const messageResult = validateConnectMessage(draft);
    if (messageResult.field || !sessionRef.current) return;
    sessionRef.current.send({ text: messageResult.value, at: Date.now() });
    setMessages(prev => [...prev, { from: 'me', text: messageResult.value }]);
    setDraft('');
  }
