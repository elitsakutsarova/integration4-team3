...
import { validateSavedMemoId } from './validators';
...
export async function addSavedMemo(authUserId, memoId) {
  const validated = validateSavedMemoId(memoId);
...
export async function mergeLocalSavedMemosIntoAccount(authUserId) {
  ...
  for (const item of pending) {
    const validated = validateSavedMemoId(item.id);
    if (!validated.value) continue;
    rows.push({
      auth_id: authUserId,
      memo_id: validated.value,
