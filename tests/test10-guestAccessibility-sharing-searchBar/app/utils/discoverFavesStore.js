...
import {
  validateDiscoverFaveId,
  validateDiscoverFaveType,
} from './validators';
...
export async function addDiscoverFave(authUserId, entry) {
  const typeResult = validateDiscoverFaveType(entry.type);
  if (typeResult.field) return { error: typeResult.message };

  const idResult = validateDiscoverFaveId(entry.id);
  if (idResult.field) return { error: idResult.message };

  const savedAt = entry.savedAt ?? new Date().toISOString();
  const fullEntry = { type: typeResult.value, id: idResult.value, savedAt };
  ...
  return withSupabaseOrLocal(
    ...
    async (client, accountId) => {
      ...
      if (faves.some(item => item.type === fullEntry.type && item.id === fullEntry.id)) {
        return { added: false, faves };
      }

      const table = tableForType(fullEntry.type);
      const idColumn = idColumnForType(fullEntry.type);
      const { error } = await client.from(table).insert({
        auth_id: accountId,
        [idColumn]: fullEntry.id,
        saved_at: savedAt,
      });
...
export async function removeDiscoverFave(authUserId, type, id) {
  const typeResult = validateDiscoverFaveType(type);
  if (typeResult.field) return { error: typeResult.message };

  const idResult = validateDiscoverFaveId(id);
  if (idResult.field) return { error: idResult.message };
  ...
  return withSupabaseOrLocal(
    ...
    async (client, accountId) => {
      ...
      const next = faves.filter(item => !(item.type === typeResult.value && item.id === idResult.value));
      if (next.length === faves.length) {
        return { faves };
      }

      const table = tableForType(typeResult.value);
      const idColumn = idColumnForType(typeResult.value);
      const { error } = await client
        .from(table)
        .delete()
        .eq('auth_id', accountId)
        .eq(idColumn, idResult.value);

      if (error) return { error };

      return { faves: next };
    },
    () => ({ faves: removeLocalDiscoverFave(authUserId, typeResult.value, idResult.value) }),
  );
}
...
export async function mergeLocalDiscoverFavesIntoAccount(authUserId) {
  ...
  for (const item of pending) {
    const typeResult = validateDiscoverFaveType(item.type);
    const idResult = validateDiscoverFaveId(item.id);
    if (!typeResult.value || !idResult.value) continue;

    const row = { auth_id: authUserId, saved_at: item.savedAt };
    if (typeResult.value === 'event') {
      eventRows.push({ ...row, event_id: idResult.value });
    } else {
      placeRows.push({ ...row, place_id: idResult.value });
    }
  }

  if (!eventRows.length && !placeRows.length) return { merged: 0 };
  ...
  return { merged: eventRows.length + placeRows.length };
}
