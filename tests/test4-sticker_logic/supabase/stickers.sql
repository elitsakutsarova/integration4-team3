...
create or replace function public.pick_pool_sticker(p_user_key text, p_location_id text)
returns text
language plpgsql
stable
as $$
...
begin
  ...
  v_idx := (v_hash % array_length(v_pool, 1)) + 1;
  return v_pool[v_idx];
end;
$$;
...
begin
  ...
  v_sticker_id := public.pick_pool_sticker(v_user_key, p_location_id);
