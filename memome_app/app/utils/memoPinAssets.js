  random: '<path d="M1.77837 1.77831V14.2223H14.2224V1.77831H1.77837ZM0.000663554 1.77831C0.000663554 1.30683 0 0 0 0C0 0 1.3069 0.000599759 1.77837 0.000599759H14.2224C14.6938 0.000599759 16.0001 0.000599759 16.0001 0.000599759C16.0001 0.000599759 16.0001 1.30683 16.0001 1.77831V14.2223C16.0001 14.6938 16.0001 15.9994 16.0001 15.9994C16.0001 15.9994 14.6938 16 14.2224 16H1.77837C1.3069 16 0 15.9994 0 15.9994C0 15.9994 0.000663554 14.6938 0.000663554 14.2223V1.77831ZM8.00036 9.33358C8.00036 9.33358 8.4163 8.91638 8.66634 8.66634C8.91638 8.4163 9.33364 8.0003 9.33364 8.0003C9.33364 8.0003 8.91638 7.5831 8.66634 7.33306C8.4163 7.08302 8.00036 6.66702 8.00036 6.66702C8.00036 6.66702 7.5831 7.08302 7.33306 7.33306C7.08302 7.5831 6.66708 8.0003 6.66708 8.0003C6.66708 8.0003 7.08302 8.4163 7.33306 8.66634C7.5831 8.91638 8.00036 9.33358 8.00036 9.33358Z" fill="currentColor"/>',
};

export function resolveMemoPinKey(pin) {
  const primaryTag = pin?.tags?.[0];
  return memoTagIconKey(primaryTag ?? 'Random');
}

export function resolveMemoPinSvg(pin) {
  const key = resolveMemoPinKey(pin);
  return MEMO_PIN_SVG[key] ?? MEMO_PIN_SVG.random;
}

function categoryIconViewBox(iconKey) {
  if (iconKey === 'fashion') return '0 0 14 16';
  if (iconKey === 'music') return '0 0 12 16';
  if (iconKey === 'nature') return '0 0 14.4 16';
  if (iconKey === 'art' || iconKey === 'random') return '0 0 16 16';
  return '0 0 14 14';
}

function categoryIconSvg(iconKey, { size, color = 'currentColor' }) {
  const resolvedKey = BADGE_ICONS[iconKey] ? iconKey : 'random';
  const path = BADGE_ICONS[resolvedKey].replace(/fill="currentColor"/g, `fill="${color}"`);
  const viewBox = categoryIconViewBox(resolvedKey);
  const sizeAttr = size ? ` width="${size}" height="${size}"` : '';
  return `<svg xmlns="http://www.w3.org/2000/svg"${sizeAttr} viewBox="${viewBox}" aria-hidden="true">${path}</svg>`;
}

export function memoPinBadgeSvg(key) {
  return categoryIconSvg(key, {});
}

export function eventCenterIconSvg(tag) {
  const key = memoTagIconKey(tag ?? 'Random');
  return categoryIconSvg(key, { size: 14, color: '#202020' });
}

export function resolveEventIconTag(pin) {
...
export function readBlobMediaDimensions(url, isVideo) {
  return readMediaDimensions(url, { isVideo });
}

export function applyPolaroidOrientationClass(element, width, height) {
  if (!element) return false;
  const orientation = orientationFromDimensions(width, height);
  if (!orientation) return false;

  element.classList.toggle('pin-memory-polaroid--horizontal', orientation === 'horizontal');
...
export function memoryPinDimensions(pin) {
  if (pinHasMedia(pin)) {
    return {
      iconSize: [POLAROID_PIN_WIDTH, POLAROID_PIN_HEIGHT],
      iconAnchor: [TAG_PIN_WIDTH / 2, POLAROID_PIN_HEIGHT],
      popupAnchor: [0, -POLAROID_PIN_HEIGHT - 4],
    };
  }
  ...
  return {
    ...
    popupAnchor: [0, -TAG_PIN_HEIGHT - 2],
  };
}
